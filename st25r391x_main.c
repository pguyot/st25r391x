...
static int st25r391x_read(struct file *file, char __user *buffer, size_t len, loff_t *offset);
static unsigned int st25r391x_poll(struct file *file, poll_table *wait);
static long st25r391x_unlocked_ioctl(struct file *file, unsigned int, unsigned long);

static int st25r391x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id);
...
static void st25r391x_transition_to_idle(struct st25r391x_i2c_data *priv) {
    ...
    priv->mode = mode_idle;
    idle_response_header.message_type = NFC_IDLE_MODE_ACKNOWLEDGE_MESSAGE_TYPE;
    idle_response_header.payload_length = 0;
    st25r391x_write_to_device(priv, (const u8*) &idle_response_header, sizeof(idle_response_header));
...
void st25r391x_process_selected_tag(struct st25r391x_i2c_data *priv, const struct nfc_detected_tag_message_payload *tag_payload, u8 cid) {
    ...
    switch(tag_payload->tag_type) {
        ...
        case NFC_TAG_TYPE_ISO14443A_NFCDEP:
            ...
            payload_len = sizeof(tag_payload->tag_type) + sizeof(tag_payload->tag_info.iso14443a) - sizeof(tag_payload->tag_info.iso14443a.uid) + tag_payload->tag_info.iso14443a.uid_len;
            break;
        ...
        case NFC_TAG_TYPE_ISO14443A_T4T_NFCDEP:
            ...
            payload_len = sizeof(tag_payload->tag_type) + sizeof(tag_payload->tag_info.iso14443a4) - sizeof(tag_payload->tag_info.iso14443a4.ats) + tag_payload->tag_info.iso14443a4.ats_len;
            break;

        case NFC_TAG_TYPE_ISO14443B:
            uid = tag_payload->tag_info.iso14443b.pupi;
            uid_len = sizeof(tag_payload->tag_info.iso14443b.pupi);
            payload_len = sizeof(tag_payload->tag_type) + sizeof(tag_payload->tag_info.iso14443b);
            break;
    ...
    if (select_tag) {
        ...
        priv->mode_params.selected.tag_id.tag_type = tag_payload->tag_type;
        priv->mode_params.selected.tag_id.cid = cid;
        priv->mode_params.selected.tag_id.uid_len = uid_len;
        memcpy((void*) priv->mode_params.selected.tag_id.uid, uid, uid_len);
        memset(priv->mode_params.selected.tag_id.uid + uid_len, 0, sizeof(priv->mode_params.selected.tag_id.uid) - uid_len);
        stop_polling_timer(priv);
    } else {
...
static void st25r391x_do_discover(struct st25r391x_i2c_data *priv) {
    if (st25r391x_turn_field_on(priv) < 0) return;

    // Technology depends on the current mode.
    if (priv->mode == mode_discover &&
        priv->mode_params.discover.protocols & (
            NFC_TAG_PROTOCOL_ISO14443A | NFC_TAG_PROTOCOL_ISO14443A_T2T | NFC_TAG_PROTOCOL_MIFARE_CLASSIC | NFC_TAG_PROTOCOL_ISO14443A_NFCDEP
            | NFC_TAG_PROTOCOL_ISO14443A4 | NFC_TAG_PROTOCOL_ISO14443A_T4T | NFC_TAG_PROTOCOL_ISO14443A_T4T_NFCDEP)) {
    ...
    if (priv->mode == mode_discover &&
        priv->mode_params.discover.protocols & (NFC_TAG_PROTOCOL_NFCF | NFC_TAG_PROTOCOL_NFCF_NFCDEP)) {
        // Passive poll ST25TB
...
static void st25r391x_do_select(struct st25r391x_i2c_data *priv) {
    ...
    // Technology depends on the current mode.
    if (priv->mode_params.select.tag_id.tag_type >= NFC_TAG_TYPE_ISO14443A && priv->mode_params.select.tag_id.tag_type <= NFC_TAG_TYPE_ISO14443A_T4T_NFCDEP) {
        st25r391x_nfca_select(priv);
    } else if (priv->mode_params.select.tag_id.tag_type == NFC_TAG_TYPE_ISO14443B) {
        st25r391x_nfcb_select(priv);
    } else if (priv->mode_params.select.tag_id.tag_type == NFC_TAG_TYPE_ST25TB) {
...
static void st25r391x_do_poll(struct work_struct *work) {
    struct st25r391x_i2c_data *priv = container_of(work, struct st25r391x_i2c_data, polling_work);
...
static int st25r391x_read(struct file *file, char __user *buffer, size_t len, loff_t *ppos) {
...
static void st25r391x_write_process_packet(struct st25r391x_i2c_data *priv, u16 payload_len) {
    uint8_t message_type = ((struct nfc_message_header*) priv->write_buffer)->message_type;
    switch (message_type) {
        case NFC_IDENTIFY_REQUEST_MESSAGE_TYPE: {
            ...
            identity_response_header.message_type = NFC_IDENTIFY_RESPONSE_MESSAGE_TYPE;
            identity_response_header.payload_length = identity_payload_len;
            st25r391x_write_to_device(priv, (const u8*) &identity_response_header, sizeof(identity_response_header));
            st25r391x_write_to_device(priv, CHIP_MODEL_IDENTITY, identity_payload_len);
            break;
        }
        ...
        case NFC_DISCOVER_MODE_REQUEST_MESSAGE_TYPE: {
            const struct nfc_discover_mode_request_message_payload* payload = (const struct nfc_discover_mode_request_message_payload*) (priv->write_buffer + sizeof(struct nfc_message_header));
            priv->mode_params.discover.protocols = payload->protocols;
            priv->mode_params.discover.polling_period = payload->polling_period;
        ...
        case NFC_SELECT_TAG_MESSAGE_TYPE: {
            const struct nfc_select_tag_message_payload* payload = (const struct nfc_select_tag_message_payload*) (priv->write_buffer + sizeof(struct nfc_message_header));
            memset(&priv->mode_params.select, 0, sizeof(priv->mode_params.select));
            priv->mode_params.select.tag_id.tag_type = payload->tag_type;
            if (payload->tag_type >= NFC_TAG_TYPE_ISO14443A && payload->tag_type <= NFC_TAG_TYPE_ISO14443A_T4T_NFCDEP) {
                priv->mode_params.select.tag_id.uid_len = payload->tag_id.iso14443a.uid_len;
                memcpy(priv->mode_params.select.tag_id.uid, (const void*) payload->tag_id.iso14443a.uid, payload->tag_id.iso14443a.uid_len);
            } else if (payload->tag_type == NFC_TAG_TYPE_ISO14443B) {
                priv->mode_params.select.tag_id.uid_len = sizeof(payload->tag_id.iso14443b.pupi);
                memcpy(priv->mode_params.select.tag_id.uid, (const void*) payload->tag_id.iso14443b.pupi, sizeof(payload->tag_id.iso14443b.pupi));
        ...
        case NFC_TRANSCEIVE_FRAME_REQUEST_MESSAGE_TYPE: {
            ...
            trigger_polling_work(priv);
            priv->running_command = 1;  // Block further commands until this one is executed.
            break;
        }
    }
}

static int st25r391x_write_bytes(struct st25r391x_i2c_data *priv, const char __user *buffer, size_t buffer_len, size_t count) {
    size_t actual = count > buffer_len ? buffer_len : count;
    if (copy_from_user(priv->write_buffer + priv->write_offset, buffer, actual)) return -EFAULT;
    priv->write_offset += actual;
    return actual;
}

static int st25r391x_write(struct file *file, const char __user *buffer, size_t len, loff_t *ppos) {
    struct st25r391x_i2c_data *priv = (struct st25r391x_i2c_data *) file->private_data;
    int written_count = 0;
    if (len <= 0) {
        return 0;
    ...
    do {
        ...
        if (priv->write_offset < sizeof(struct nfc_message_header)) {
            written_count = st25r391x_write_bytes(priv, buffer, len, sizeof(struct nfc_message_header) - priv->write_offset);
            if (written_count < 0) break;
            len -= written_count;
            buffer += written_count;
        }
        payload_len = ((struct nfc_message_header*) priv->write_buffer)->payload_length;
        if (priv->write_offset < payload_len + sizeof(struct nfc_message_header)) {
            int payload_written_count = st25r391x_write_bytes(priv, buffer, len, sizeof(struct nfc_message_header) + payload_len - priv->write_offset);
            if (payload_written_count < 0) break;
...
static long st25r391x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    // struct st25r391x_i2c_data *priv = (struct st25r391x_i2c_data *) file->private_data;
    // Fixed size commands.
    switch(cmd) {
        case NFC_RD_GET_PROTOCOL_VERSION: {
            uint64_t version = NFC_PROTOCOL_VERSION_1;
            return copy_to_user((uint64_t*) arg, &version, sizeof(version)) ? -EFAULT : 0;
        }
    }
...
static int st25r391x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    // Set default
    result = st25r391x_direct_command(i2c, ST25R391X_SET_DEFAULT_COMMAND_CODE);
    if (result < 0) {
        dev_err(dev, "st25r391x_i2c_probe: Failed to send set default command %d", result);
        return result;
    }
    ...
    result = i2c_smbus_write_i2c_block_data(i2c, ST25R391X_TEST_ACCESS_COMMAND_CODE, 2, buffer);
    if (result < 0) {
        dev_err(dev, "st25r391x_i2c_probe: Failed to write test register %d", result);
        return result;
    }

    // Configure IO Configuration Registers
    result = st25r391x_write_registers_check(i2c, ST25R391X_IO_CONFIGURATION_1_REGISTER, 2, 0, 0b00100000);
    if (result < 0) {
        dev_err(dev, "st25r391x_i2c_probe: Failed to write IO Configuration Registers %d", result);
        return result;
    }

    // Read IC identity register to make sure we have a ST25R
    result = st25r391x_read_register_byte(i2c, ST25R391X_IC_IDENTITY_REGISTER);
    if (result < 0) {
        return result;
    }
    if (result != 0b00101010) {
        dev_err(dev, "st25r391x_i2c_probe: Unexpected identity register value %d", result);
        return -1;
    }

    // Adjust regulators
    result = st25r391x_write_register_byte_check(i2c, ST25R391X_REGULATOR_VOLTAGE_CONTROL_REGISTER, 0b11110000);
    if (result < 0) {
        dev_err(dev, "st25r391x_i2c_probe: Failed to write regulator voltage control register: %d", result);
        return result;
    }
    result = st25r391x_write_register_byte_check(i2c, ST25R391X_REGULATOR_VOLTAGE_CONTROL_REGISTER, 0b01110000);
    if (result < 0) {
        dev_err(dev, "st25r391x_i2c_probe: Failed to write regulator voltage control register: %d", result);
        return result;
    }
    ...
    if (err < 0) {
        dev_err(dev, "st25r391x_i2c_probe: Failed to register character device: %d", err);
	...
	if (IS_ERR(priv->st25r391x_class)) {
        ...
        dev_err(dev, "st25r391x_i2c_probe: class_create failed: %d", err);
        st25r391x_i2c_remove(i2c);
        return err;
	}
    ...
    if (err) {
        dev_err(dev, "st25r391x_i2c_probe: Failed to add cdev: %d", err);
        st25r391x_i2c_remove(i2c);
        return err;
    }

	priv->device = device_create(priv->st25r391x_class, dev, priv->chrdev, NULL, /* no additional data */ DEVICE_NAME "%d", MINOR(priv->chrdev));
	if (IS_ERR(priv->device)) {
		err = PTR_ERR(priv->device);
        dev_err(dev, "st25r391x_i2c_probe: Failed to create device: %d", err);
        st25r391x_i2c_remove(i2c);
        return err;
    }
...
static int st25r391x_i2c_remove(struct i2c_client *client)
{
    ...
    if (priv->chrdev) {
        if (priv->st25r391x_class) {
            if (priv->cdev.ops) {
                device_destroy(priv->st25r391x_class, MKDEV(MAJOR(priv->chrdev), MINOR(priv->chrdev)));
                cdev_del(&priv->cdev);
            }
